        ...
        if (response === false) {
            ...
            switch (response.type) {
                case NodeFinishCollectResponseType.Failed: {
                    embed
                        .setTitle("Failed")
                        .setDescription(`You've been ${NodeAction[response.node.type]} for ${TimeParser.parseToString(response.elapsed)} but did not get any materials...`)
                    break
                }
                ...
                case NodeFinishCollectResponseType.Success: {
                    embed
                        ...
                        .setDescription(`You've been ${NodeAction[response.node.type]} at ${response.node.name} for ${TimeParser.parseToString(response.elapsed)} and received the following materials:\n${response.results.map(
